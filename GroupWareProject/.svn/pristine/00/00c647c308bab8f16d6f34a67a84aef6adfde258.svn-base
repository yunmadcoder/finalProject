						...
						<ul id="add-line-area">
							<c:forEach var="sanctioner" items="${approvalVO.sanctionerList }">
								<c:if test="${sanctioner.sanctnerNo ne 0}">
									<li class="py-2 px-3 position-relative <c:if test="${sanctioner.sanctnerSttusCode eq 'G301'}">current</c:if>">
										<span class="photo d-inline-block position-absolute"> <a href="#">
	...
	<div class="interactive-modal-content" style="width: 800px;">
		...
		<div class="content">
			<nav class="nav nav-underline nav-phils nav-fill mb-3" style="">
				...
				<a href="#" class="nav-link rounded-0 fw-semibold" id="nav-info-tab" data-bs-toggle="tab" data-bs-target="#nav-info" type="button" role="tab" aria-controls="nav-info" aria-selected="false">결재정보</a>
			</nav>
			<div class="tab-content" id="nav-tabContent">
				<div class="tab-pane fade show active" id="nav-draft" role="tabpanel" aria-labelledby="nav-draft-tab" tabindex="0">
					<table class="table table-borderless">
						<tr>
							<th>결재문서명</th>
							<td>
								<span class="font-weight-bold" id="aprovlNm"></span>
							</td>
						</tr>
						<tr>
							<th>기안의견</th>
						...
						<tr>
							<th>긴급문서</th>
							<td>
								<span> <input type="checkbox" class="form-check-input" id="isEmergency"> <label for="isEmergency">긴급</label>
											...
											<c:forEach var="sanctioner" items="${approvalVO.sanctionerList }">
												<c:if test="${sanctioner.sanctnerNo ne 0}">
													<tr data-no="${sanctioner.approverNo }">
														...
														<td>
															<a href="#" data-del-no="${sanctioner.approverNo }">
										...
										<c:if test="${not empty approvalVO.rfrncList }">
											<c:forEach var="rfrnc" items="${approvalVO.rfrncList }">
												<tr data-rfrncer-no="${rfrnc.rfrncerNo }">
													...
													<td>${isChecked }</td>
													<td>
														<a href="#" data-del-no="${rfrnc.rfrncerNo }">
															<i class="fa-sharp fa-regular fa-trash"></i>
														</a>
													</td>
												</tr>
				...
				<div class="text-end p-0 pt-3">
					<input type="button" class="btn btn-sm btn-secondary rounded-0" value="결재요청" id="confrimDraftBtn">
					<input type="button" class="btn btn-sm rounded-0" value="취소" style="border: 1px solid #ddd;" id="cancelDraftModal">
				</div>
			</div>
		</div>
	</div>
	...
	<div class="interactive-modal-content" style="width: 300px;">
		...
		<div class="card-body rounded-0 p-0">
			<div class="input-group justify-content-center align-items-center">
				<label class="fw-semibold me-3" for="saveLineBtn">결재선 이름</label>
				<input type="text" class="form-control rounded-0" id="lineName">
			</div>
		</div>
...
<div id="save-group-modal" class="interactive-modal" style="z-index: 9999;">
	<div class="interactive-modal-content" style="width: 300px;">
		...
		<div class="card-body rounded-0 p-0">
			<div class="input-group justify-content-center align-items-center">
				<label class="fw-semibold me-3" for="saveGroupBtn">그룹 이름</label>
				<input type="text" class="form-control rounded-0" id="groupName">
			</div>
		</div>
		<div class="card-footer text-end p-0 pt-3">
	...
	$(function() {
		...
		addBtn.on('click', function() {
			...
			for(let node of selecetdNodes) {
				...
				if(isRefer) {
					html += `
						<tr \${dataAttr}="\${node.original.emplNo}" draggable="true" class="dragitem">
		...
		$('#nav-tabContent').on('click', '[data-del-no]', function(e) {
			...
			.done(function(res) {
				...
				for(let line of res) {
					for(let data of line.sanctionerBookmarkList) {
						html += '<tr data-no="'+data.emplNo+'" draggable="true" class="dragitem">';
						html +=	'	<td>결재</td>';
		...
		function getInsertData(type) {
			...
			console.log(rfrncList);
			console.log($(rfrncList[0]).data('rfrncerNo'));
