											...
											<tr id="${empl.emplNo }">
												...
												<c:if test="${empl.enabled  eq 1}">
													<td style="text-align: left;">
														<h6 class="text-dark">
															정상
															<a class="mb-0" href="#" onclick="enabled(${empl.emplNo},'${empl.emplNm}')">[중지]</a>
														</h6>
													</td>
												</c:if>
													...
													<c:when test="${empl.authList.size() eq 2}">
														<td style="text-align: left;">
															<h6 class="text-dark" id="adminStatus${empl.emplNo}">
																Y
																<a class="mb-0" href="#" onclick="deleteAuth(${empl.emplNo},'${empl.emplNm}')"> [삭제]</a>
															</h6>
														</td>
													</c:when>
													<c:otherwise>
														<td style="text-align: left;">
															<h6 class="text-dark" id="adminStatus${empl.emplNo}">
																N
																<a class="mb-0" href="#" onclick="insertAuth(${empl.emplNo},'${empl.emplNm}')"> [지정]</a>
															</h6>
														</td>
													</c:otherwise>
												</c:choose>
...
function enabled(emplNo,emplNm) {
	
	showConfirm(emplNm+" 사원을 이용중지하시겠습니까?",'','question').then(result=> {
...
function deleteAuth(emplNo,emplNm) {
	
	showConfirm(emplNm+" 사원을 관리자에서\n삭제하시겠습니까?",'','question').then(result=> {
		if(result.isConfirmed){
			$.ajax({
...
function insertAuth(emplNo,emplNm) {
	
	showConfirm(emplNm+" 사원을 관리자로\n지정하시겠습니까?",'','question').then(result=> {
		if(result.isConfirmed){
			$.ajax({
