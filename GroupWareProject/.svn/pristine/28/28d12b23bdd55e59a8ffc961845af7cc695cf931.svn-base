											...
											<tr id="${empl.emplNo }">
												...
												<c:if test="${empl.enabled  eq 0}">
													<td style="text-align: left;">
														<h6 class="text-dark">
															이용중지
															<a class="mb-0" href="#" onclick="enabled(${empl.emplNo},'${empl.emplNm}')">[해제]</a>
														</h6>
													</td>
												</c:if>
												<td style="text-align: left;">
													<h6 style="color: red;">
														<a class="mb-0" href="#" onclick="openModal(${empl.emplNo},'${empl.emplNm}')"> [삭제]</a>
													</h6>
												</td>
											</tr>
										</c:forEach>
...
function openModal(emplNo, emplNm) {
	$('#bs-example-modal-md').modal('show');
	$("#emplNmArea").html(`<i class='fa-duotone fa-triangle-exclamation'></i> \${emplNm } 사원을 삭제합니다.`);
...
function enabled(emplNo, emplNm) {
	...
	showConfirm(emplNm+" 사원의 이용중지를 해제하시겠습니까?",'','question').then(result=> {
		if(result.isConfirmed){
			$.ajax({
